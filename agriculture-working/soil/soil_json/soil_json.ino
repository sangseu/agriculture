...
#define pfodWifiWebConfigAP "esp_0000000"
...
void setup ( void ) {
  /*
  String s = getID();
  String s1 = "viota/" + s + "out";
  String s2 = "viota/" + s + "in";
  const char* topic_pub = &s1[0];
  */
  WiFi.mode(WIFI_STA);
...
void loop() {
  ...
  //JsonObject& m_gpio = jsonBuffer.createObject();
...
void callback(char* topic, byte* payload, unsigned int length)
{
  ...
  String v_gpio = root2["gpio"];
  String v_get = root2["get"];
  ...
  if (v_gpio.length()) {
    ...
    char* c_v_gpio = &v_gpio[0];

    JsonObject& j_gpio2 = jsonBuffer.parseObject(c_v_gpio);

    ID_relay = j_gpio2["ID"];
    wrt_relay = j_gpio2["write"];
