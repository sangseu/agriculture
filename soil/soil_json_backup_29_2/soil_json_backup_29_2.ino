...
const uint16_t t_retry_conn_wifi = 180; //(s)
...
StaticJsonBuffer<100> jsonBuffer_data;
StaticJsonBuffer<70> jsonBuffer_stt;
StaticJsonBuffer<50> jsonBuffer_m_gpio;

JsonObject& root = jsonBuffer.createObject();
JsonObject& data = jsonBuffer_data.createObject();
JsonObject& stt = jsonBuffer_stt.createObject();
JsonObject& m_gpio = jsonBuffer_m_gpio.createObject();
*/
long lastMsg = 0;
...
void loop() {
  ...
  if (now - lastMsg > f_pub)
  {
    ...
    lightMeter.begin();
    f_lux = lightMeter.readLightLevel();
    ...
    Serial.print("[1]");
    data.printTo(Serial); Serial.println();
    if ( 0 < (int)f_lux && (int)f_lux < 50000 ) {
      data["lux"] = f_lux;
    }
    else {
    ...
    root.remove("status");
    root["data"] = data;//just pub "data"
    String json;
    root.printTo(json);
    char* c_json = &json[0];
    Serial.print(c_json);
    clientPS.publish(mqtt_topic_pub_soi, c_json);
...
void callback(char* topic, byte* payload, unsigned int length)
{
  ...
  StaticJsonBuffer<200> jsonBuffer;
  StaticJsonBuffer<100> jsonBuffer_cmd;
  StaticJsonBuffer<70> jsonBuffer_stt;
  ...
  /*
  aJsonObject* rcv_root = aJson.parse(c_json);
  if (rcv_root != NULL) {
    aJsonObject* command = aJson.getObjectItem(rcv_root, "command");
    if (command != NULL) {
      ...
      aJsonObject* get_stt = aJson.getObjectItem(command, "get");
      if (get_stt != NULL) {
        get_status = 1;
      }
      else get_status = 0;
  ...
  aJson.deleteItem(rcv_root);
  */
  JsonObject& root = jsonBuffer.parseObject(s_json);
  String s_cmd = root["command"];
  Serial.println(s_cmd);

  JsonObject& cmd = jsonBuffer_cmd.parseObject(s_cmd);
  String s_gpio = cmd["gpio"];
  Serial.println(s_gpio);

  String s_get = cmd["get"];
  Serial.println(s_get);

  Serial.println("[parse gpio]");
  if (s_gpio) {
    char* c_gpio = &s_gpio[0];

    aJsonObject* gpio = aJson.parse(c_gpio);
    aJsonObject* ID = aJson.getObjectItem(gpio, "ID");
    aJsonObject* wrt = aJson.getObjectItem(gpio, "write");
    ID_relay = ID->valueint;
    wrt_relay = wrt->valueint;
  ...
  if (get_status)
  {
    String list = String(readgpio(), HEX);
    Serial.print("status ");
    Serial.println(list);
    m_gpio["lst"] = list;
    m_gpio["stt"] = 1;
    stt["m_gpio"] = m_gpio;
    root.remove("data");//just pub "status"
    root["status"] = stt;
...
{
  byte r = 0;
  r |= digitalRead(relay1) << 1;
  r |= digitalRead(relay2) << 2;
  return r;
}
...
void re_setup()
{
  setupWifi();
  setupMqtt();
  clientPS.connect(ID_esp);
}

String getID() {
  uint8_t mac[] = {0, 0, 0, 0, 0, 0};
  String s;
  WiFi.macAddress(mac);//5ccf7fa2d42
